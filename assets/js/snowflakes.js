...
function setup() {
  ...
  resetSnowflakes();
  canvas.onclick = resetSnowflakes;
...
function draw() {
  ...
  ctx.beginPath();
  let gradient = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, vmin);
  gradient.addColorStop(0, "#aa2020");
  gradient.addColorStop(1, "#501515");
...
function Snowflake() {
  ...
  this.render = function (ctx) {
    ...
    while (theta < Math.PI * 2 + Math.PI / this.raysCount) {
      ...
      for (i = 0; i < this.rectCount; i++) {
        ...
        ctx.fillRect(-rectWidth / 2, -rectWidth / 2, rectWidth, rectWidth);
        ctx.closePath();
        ctx.restore();
      }
      ctx.closePath();
  ...
  this.reset = function () {
    ...
    if (this.posX > w + this.size * 2 || this.posX < 0 - this.size * 2) {
      this.posX = Math.random() * w;
    }
  };
}
